...
function Header() {
  ...
  useEffect(() => {
    ...
    if (responseJsonData[responseJsonData.length - 3]) {
      setPercentage(
        responseJsonData[responseJsonData.length - 3].percentagecorrect
      );
    }
    if (responseJsonData[responseJsonData.length - 2]) {
      setAverageWoW(
        responseJsonData[responseJsonData.length - 2].average_growth
      );
    }
    if(responseJsonData[responseJsonData.length - 3]){
    setTotalScore(responseJsonData[responseJsonData.length - 3].finalscore);
    }
    console.log("Loaded test respones data in dashboard function successfully!");
  }, [responseJsonData]);
